...
def main ():
    ...
    train_dataloader, test_dataloader, scaler_x, scaler_y  = preprocess_data(x_data, temp_data, elev_data, y_data, seq_length, batch_size)
...
def preprocess_data(x_data, temp_data, elev_data, y_data, seq_length, batch_size, train_split=0.8):
    ...
    # Broadcast elev_data to match the number of samples
    elev_data_broadcasted = np.tile(elev_data, (num_samples, 1, 1))  # Shape: (num_samples, height, width)
    elev_data_reshaped = elev_data_broadcasted.reshape(num_samples, -1)  # Shape: (num_samples, height * width)

    # Stack the features along the last axis
    combined_data = np.stack([x_data_reshaped, temp_data_reshaped, elev_data_reshaped], axis=-1)  # Shape: (num_samples, height * width, 3)
    ...
    combined_data_normalized = combined_data_normalized.reshape(num_samples, 3, height, width)  # 3 channels: precipitation, temperature, elevation
    ...
    x_sequences = np.array(x_sequences)  # Shape: (num_sequences, seq_length, 3, height, width)
